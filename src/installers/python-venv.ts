...
async function findSystemPython(): Promise<string> {
  ...
  throw new Error(
    `wide-researcher needs Python ≥ ${MIN_PY[0]}.${MIN_PY[1]}. None found on PATH.\n` +
...
async function venvIsHealthy(): Promise<boolean> {
  if (!(await exists(venvPython()))) return false;
  try {
...
export async function installPythonVenv(opts: InstallVenvOptions = {}): Promise<void> {
  ...
  if (!(await venvIsHealthy())) {
    throw new Error(
      `venv created but health-check failed. Inspect ${venvRoot()} and re-run with --force.`,
    );
  }
