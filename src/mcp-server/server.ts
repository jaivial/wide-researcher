...
const TOOLS = [
  ...
  },
  {
    name: 'wr_collections',
    description:
      'List every Qdrant collection on the server with status, points_count, and vector dim. The current project default is flagged with is_default. Use this to discover which value to pass as the `collection` override on other tools (wr_find, wr_impact, wr_symbol_find, etc.).',
    inputSchema: {
      type: 'object',
      properties: {
        filter: {
  ...
  },
  {
    ...
    description:
      'Read-only search over the shared memories collection. Use it to list or search stored memory notes by title, summary, kind, tags, and related files.',
    inputSchema: {
      type: 'object',
      properties: {
        query: {
  ...
  },
  {
    ...
    description:
      'Add a new memory note to the shared memories collection. Idempotent — re-adding the same title+content reuses the deterministic point id. Fields: title (required), content (required), summary, kind, tags, files, learned_at, resolved.',
    inputSchema: {
      type: 'object',
      properties: {
...
for (const tool of TOOLS) {
  if (COLLECTION_OVERRIDE_TOOLS.has(tool.name)) {
    (tool.inputSchema.properties as Record<string, unknown>).collection = {
      type: 'string',
      description:
        'Optional Qdrant collection override (base name). Defaults to the project collection. Call wr_collections to discover available names.',
    };
  }
}
...
const HANDLERS: Record<string, Handler> = {
  ...
  wr_impact: async (a) =>
    withResultCache(
      { tool: 'wr_impact', d: a.description ?? '', k: a.k ?? 15, rerank: !RERANK_DISABLED, collection: a.collection ?? null },
      async () => {
  ...
  wr_arch_impact: async (a) =>
    withResultCache(
      { tool: 'wr_arch_impact', d: a.description ?? '', k: a.k ?? 15, rerank: !RERANK_DISABLED, collection: a.collection ?? null },
      async () => {
  ...
  wr_index_status: async (a) =>
    jsonContent({ tool: 'wr_index_status', ...(await wrIndexStatus(a.collection ?? null)) }),
  wr_collections: async (a) =>
    jsonContent({ tool: 'wr_collections', ...(await wrCollections({ filter: a.filter ?? null })) }, responseBudget(a)),
  wr_memories_find: async (a) =>
    jsonContent(
      {
  ...
  wr_memories_add: async (a) => {
    const result = await wrMemoryAdd(embed, {
      ...
      resolved: a.resolved as boolean | undefined,
    });
    return jsonContent(
      { tool: 'wr_memories_add', ...result },
      responseBudget(a),
    );
  },
