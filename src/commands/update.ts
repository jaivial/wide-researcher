...
import { modelById, type EmbedProvider } from '../models/registry.js';
...
async function readPersistedConfig(configPath: string): Promise<PersistedConfig | null> {
  ...
  try {
    ...
    throw new Error(
      `existing project config at ${configPath} is not valid JSON: ${(e as Error).message}`,
    );
  }
}

async function pipUpgrade(): Promise<void> {
  if (!(await exists(venvPython()))) {
    log.warn(
      `python venv missing at ${venvRoot()} — run 'wide-researcher init' instead of 'update' on this machine.`,
    );
    return;
  }
...
async function restartIndexerService(slug: string): Promise<void> {
  if (hasSystemd()) {
    ...
    try {
      ...
      log.warn(`could not restart ${unit}: ${(e as Error).message}`);
    }
    return;
  }
  if (hasLaunchd()) {
    ...
    try {
      ...
      log.warn(`could not kickstart ${label}: ${(e as Error).message}`);
    }
    return;
  }
...
export async function runUpdate(opts: UpdateOptions = {}): Promise<void> {
  ...
  if (!persisted) {
    throw new Error(
      `no project config found at ${id.configPath}. ` +
        `Run 'wide-researcher init' on this project first; 'update' is only ` +
        `for projects that have been initialised at least once.`,
    );
  }
  const providerKey = persisted.embed_provider as EmbedProvider | undefined;
  if (!providerKey) {
    throw new Error(
      `project config ${id.configPath} is missing 'embed_provider'. ` +
