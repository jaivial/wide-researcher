...
async function createSchema(session: Session): Promise<void> {
...
async function scrollPayloads(qdrant: QdrantClient, collection: string, maxFiles: number): Promise<QdrantPoint[]> {
  ...
  while (true) {
    ...
    for (const point of res.points ?? []) {
      points.push(point);
      if (maxFiles > 0 && new Set(points.map((p) => p.payload?.file_path)).size >= maxFiles) return points;
    }
    if (res.next_page_offset === undefined || res.next_page_offset === null) break;
    offset = res.next_page_offset;
  }
  return points;
}

async function syncChunk(session: Session, payload: Record<string, unknown>): Promise<void> {
  const filePath = String(payload.file_path ?? '');
  if (!filePath) return;
  const language = String(payload.language ?? 'text');
  await session.run(
    `MERGE (f:File {path: $filePath})
     SET f.repo = $repo, f.language = $language`,
    { filePath, repo: String(payload.repo ?? ''), language },
  );
  ...
  for (const target of asStringArray(payload.calls)) {
    await session.run(
      `MATCH (f:File {path: $filePath})-[:DECLARES]->(s:Symbol)
       ...
       MERGE (n:Name {name: $target})
       MERGE (s)-[:CALLS]->(n)`,
      { filePath, line: payload.start_line ?? 0, target },
    );
  }
  ...
  for (const target of asStringArray(payload.implements)) {
    await session.run(
      `MATCH (f:File {path: $filePath})-[:DECLARES]->(s:Symbol)
       ...
       WITH s LIMIT 1
       MERGE (n:Name {name: $target})
       MERGE (s)-[:IMPLEMENTS]->(n)`,
      { filePath, line: payload.start_line ?? 0, target },
    );
  }
}

async function sync(driver: Driver, qdrant: QdrantClient, collection: string, database: string | undefined, maxFiles: number): Promise<number> {
  const session = driver.session(database ? { database } : undefined);
  try {
    await createSchema(session);
    const points = await scrollPayloads(qdrant, collection, maxFiles);
    for (const point of points) {
      if (point.payload) await syncChunk(session, point.payload);
    }
...
export async function runNeo4jSync(opts: Neo4jSyncOptions = {}): Promise<void> {
