...
import { ensureDir, exists, qdrantBinary, qdrantConfigPath, qdrantRoot, qdrantStorageRoot, } from '../utils/paths.js';
...
function archiveUrl() {
    ...
    return {
        url: `https://github.com/qdrant/qdrant/releases/download/v${QDRANT_VERSION}` +
            `/qdrant-${qdrantTriple}.${qdrantArchiveExt}`,
        ext: qdrantArchiveExt,
    };
}
async function downloadArchive(toFile, url) {
...
export async function installQdrant(opts = {}) {
    ...
    if (!(await exists(qdrantBinary()))) {
        throw new Error(`qdrant binary not found after extraction in ${qdrantRoot()}. Archive layout may have changed.`);
    }
