...
import { installIndexerServiceLinux, installQdrantServiceLinux, uninstallIndexerServiceLinux, uninstallQdrantServiceLinux, } from './systemd-linux.js';
import { installIndexerServiceMacOS, installQdrantServiceMacOS, uninstallIndexerServiceMacOS, uninstallQdrantServiceMacOS, } from './launchd-macos.js';
function warnWindowsManual() {
...
export async function installQdrantSupervisor(opts = {}) {
    if (hasSystemd())
...
export async function uninstallQdrantSupervisor() {
    if (hasSystemd())
...
export async function installIndexerSupervisor(opts) {
    if (hasSystemd())
        return installIndexerServiceLinux(opts);
    if (hasLaunchd())
        return installIndexerServiceMacOS(opts);
    if (isWindows()) {
...
export async function uninstallIndexerSupervisor(slug) {
    if (hasSystemd())
        return uninstallIndexerServiceLinux(slug);
    if (hasLaunchd())
        return uninstallIndexerServiceMacOS(slug);
