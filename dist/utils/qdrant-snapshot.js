...
function qdrantUrl(url = 'http://127.0.0.1:6333') {
    return url;
}
export async function getCollectionInfo(collection, url) {
    try {
 ...
 */
export async function snapshotCollection(collection, provider, url) {
    ...
    if (!res.ok) {
        throw new Error(`snapshot failed: HTTP ${res.status} ${res.statusText} from ${u}`);
    }
    const body = (await res.json());
 ...
 */
export async function findLatestBackup(slug, provider) {
 ...
 */
export async function restoreFromSnapshot(collection, snapshotPath, url) {
    const u = qdrantUrl(url);
    ...
    // file:// works.
    const recoverRes = await fetch(`${u}/collections/${collection}/snapshots/recover`, {
        method: 'PUT',
    ...
    if (!recoverRes.ok) {
        ...
        throw new Error(`snapshot restore failed: HTTP ${recoverRes.status} — ${errBody.slice(0, 300)}`);
    }
    void buf;
    log.ok(`restored ${collection} from snapshot`);
}
//# sourceMappingURL=qdrant-snapshot.js.map