...
export function loadProjectConfig() {
    const path = argFor('--project-config') ??
        process.env.WIDE_RESEARCHER_PROJECT_CONFIG ??
        null;
    if (!path) {
        throw new Error('wide-researcher MCP server: missing --project-config <path> argv flag ' +
    ...
    try {
        ...
        throw new Error(`failed to read project config ${path}: ${e.message}`);
    }
    let json;
    try {
        json = JSON.parse(raw);
        ...
        throw new Error(`project config ${path} is not valid JSON: ${e.message}`);
    }
    const project_root = (json.project_root ?? json.projectRoot);
    const collection_name = (json.collection_name ?? json.collectionName);
    if (!project_root || !collection_name) {
        throw new Error(`project config ${path} missing required keys: project_root + collection_name`);
    }
    const neo4j = (json.neo4j ?? {});
    return {
        ...
        embedModel: String(json.model_path ??
            json.modelPath ??
            json.embed_model ??
            json.embedModel ??
            'sentence-transformers/all-MiniLM-L6-v2'),
        embedDim: Number(json.embed_dim ?? json.embedDim ?? 384),
        secretsPath: json.secrets_path || json.secretsPath ? String(json.secrets_path ?? json.secretsPath) : null,
        cohereApiKeyField: String(json.cohere_api_key_field ?? json.cohereApiKeyField ?? 'cohere_api_key'),
