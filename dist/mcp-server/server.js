...
const TOOLS = [
    ...
    {
        name: 'wr_symbol_find',
        description: 'Find indexed AST/symbol graph nodes in the project symbol collection. Best for declarations, functions, classes, interfaces, exports, calls, type relations.',
        inputSchema: {
            type: 'object',
            properties: {
    ...
    {
        name: 'wr_callers',
        description: 'Find chunks whose structural graph payload calls or references the given symbol name.',
        inputSchema: {
            type: 'object',
            properties: {
    ...
    {
        name: 'wr_callees',
        description: 'Return calls emitted by chunks matching a symbol name/FQN or all chunks in a file path.',
        inputSchema: {
            type: 'object',
            properties: {
    ...
    {
        name: 'wr_importers',
        description: 'Find chunks/files importing a module string or resolved file path.',
        inputSchema: {
            type: 'object',
            properties: {
    ...
    {
        name: 'wr_arch_impact',
        description: 'Architecture impact analysis combining semantic chunk hits, symbol-node hits, callers, importers, exports, and type-relation expansion.',
        inputSchema: {
            type: 'object',
            properties: {
    ...
    {
        name: 'wr_skill_add',
        description: 'Add a markdown document to the <collection>_skills Qdrant collection. Accepts either `path` (abs .md file or directory of SKILL.md / references/*.md / agents/*.md under <project>/.claude/ or ~/.claude/) or `content` (inline markdown). Idempotent — re-adding the same source reuses the deterministic point id.',
        inputSchema: {
            type: 'object',
            properties: {
...
function fitToBudget(payload, maxBytes) {
    ...
    if (Array.isArray(obj.results) && obj.results.length > 0) {
        for (const lines of [10, 6, 3, 1]) {
            ...
            if (Buffer.byteLength(JSON.stringify(slim, null, 2), 'utf8') <= maxBytes)
                return slim;
        }
        for (const keep of [3, 2, 1]) {
            if (obj.results.length <= keep)
                break;
            const slim = {
                ...obj,
...
const HANDLERS = {
    wr_find: async (a) => withResultCache({
        ...
        return jsonContent(fitToBudget(built, responseBudget(a)), responseBudget(a));
    }),
    wr_file: async (a) => {
    ...
    wr_skill_add: async (a) => {
        ...
        return jsonContent({ tool: 'wr_skill_add', ...result }, responseBudget(a));
    },
};
/* ── Server wiring ──────────────────────────────────────────────────── */
