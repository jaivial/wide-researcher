...
_SCRIPT = """
import React, { useMemo, useCallback, useState, useRef, useEffect } from 'https://esm.sh/react@18';
import { createRoot } from 'https://esm.sh/react-dom@18/client?deps=react@18';
import ReactFlow, {
  Background, Controls, MiniMap, Handle, Position, useReactFlow,
...
function App() {
  const [nodeList, setNodeList] = useState(() => DATA.nodes);
  const [edgeList] = useState(() =>
    DATA.edges.map(e => ({
  ...
  return React.createElement(ReactFlow, {
    nodes: nodeList, edges: edgeList, nodeTypes,
    onNodesChange: null,
    fitView: true,
...
<style>
  ...
  .origin-card {{
    ...
    box-shadow: 0 8px 40px #1f6feb77;
    max-width: 420px; min-width: 280px;
    text-align: center; font-weight: 600; font-size: .95rem; line-height: 1.4;
  }}
  .origin-card small {{
